...
ChatGPT가 오늘 일정을 조회하고(`get_active_event`), 결과 카드(`make_card`)나 첨부한 스크린샷(`upload_image`)을 준비한 뒤, 미리보기를 보여주고 확인을 받아 올립니다(`post_showcase`).
...
2) get_active_event로 오늘 일정 확인 (없으면 알리고 중단)
3) 대화에 첨부된 스크린샷이 있으면 upload_image, 없으면 make_card로 이미지 준비
4) 미리보기를 보여주고 한 번만 확인받기
5) post_showcase(tool: "ChatGPT")로 올리고 eventTitle과 url 알려주기
