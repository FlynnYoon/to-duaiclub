...
`duai event` 결과의 첫 번째 일정에 올라간다(`--event` 없이 올리면 서버가 같은 규칙으로 고른다). 일정이 여러 개여도 묻지 않고 첫 번째에 올린다.
일정이 하나도 없으면 올리지 말고, 만든 파일과 요약을 보여준 뒤 "오늘 등록된 모임 일정이 없어 올리지 못했습니다"라고 알린다.
